...
export default function Watchlist() {
  ...
  function getStatus(e: WatchEntry) {
    if (e.lastChecked === undefined || e.lastChecked === null || e.breached === undefined || e.breached === null) {
      return { label: "Unscanned", color: "rgba(255,255,255,0.4)", bg: "rgba(255,255,255,0.04)", border: "rgba(255,255,255,0.08)" };
    }
    if (e.breached) {
      return { label: "Breached (" + (e.breachCount || 0) + ")", color: "#e05c4b", bg: "rgba(224,92,75,0.08)", border: "rgba(224,92,75,0.25)" };
    }
    ...
    <PageShell eyebrow="Breach monitoring" title="Watchlist" subtitle="Add emails to monitor. Scan anytime. Get alerted instantly when a new breach is detected.">
      <Card>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          ...
          <span style={{ fontSize: "13px", fontWeight: 700, color: "#6ce4c0" }}>
            {emails.length}
            <span style={{ color: "rgba(255,255,255,0.3)", fontWeight: 400, fontSize: "11px" }}>
              {" - "}{isPro ? "unlimited" : "max 3 (free)"}
            </span>
          </span>
        </div>
      </Card>
      ...
      <Card>
        ...
        <div style={{ display: "flex", gap: "8px" }}>
          ...
          />
          <button onClick={add} disabled={adding || !newEmail.includes("@")} style={{ padding: "11px 22px", fontSize: "13px", fontWeight: 700, color: "#000", background: adding || !newEmail.includes("@") ? "rgba(255,255,255,0.4)" : "#fff", border: "none", borderRadius: "10px", cursor: adding || !newEmail.includes("@") ? "not-allowed" : "pointer", fontFamily: "inherit" }}>
            {adding ? "..." : "Add"}
          </button>
        </div>
        {!isPro && emails.length >= 3 && (
          <p style={{ marginTop: "10px", fontSize: "11px", color: "#c48b20" }}>
            Free tier limit reached. <a href="/pricing" style={{ color: "#6c9ef7", textDecoration: "underline" }}>Upgrade to Pro</a> for unlimited.
          </p>
        )}
      </Card>
            ...
            <Card key={e.email}>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: "12px" }}>
                ...
                <div style={{ display: "flex", gap: "6px" }}>
                  <button onClick={() => scan(e.email)} disabled={scanning === e.email} style={{ padding: "8px 14px", fontSize: "12px", fontWeight: 600, color: "#6c9ef7", background: "rgba(108,158,247,0.06)", border: "1px solid rgba(108,158,247,0.25)", borderRadius: "8px", cursor: scanning === e.email ? "not-allowed" : "pointer", fontFamily: "inherit" }}>
                    {scanning === e.email ? "..." : "Scan"}
                  </button>
                  <button onClick={() => remove(e.email)} style={{ padding: "8px 14px", fontSize: "12px", fontWeight: 600, color: "#e05c4b", background: "rgba(224,92,75,0.06)", border: "1px solid rgba(224,92,75,0.25)", borderRadius: "8px", cursor: "pointer", fontFamily: "inherit" }}>
                    Remove
                  </button>
                </div>
              </div>
